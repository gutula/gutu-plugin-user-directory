# User Directory Developer Guide
...
- You need **directory records**, **people projection**, **searchable identities** as a governed domain boundary.
...
| `framework/builtin-plugins/user-directory` | Nested publishable plugin package. |
| `framework/builtin-plugins/user-directory/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/user-directory/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/user-directory/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/user-directory/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/user-directory/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/user-directory` |
| Manifest ID | `user-directory` |
| Display Name | User Directory |
| Domain Group | Operational Data |
| Default Category | User Management / Directory & Profiles |
| Version | `0.1.0` |
| Kind | `app` |
...
| Provides Capabilities | `directory.people` |
| Owns Data | `directory.people` |
...
| --- | --- | --- | --- |
| Action | `directory.people.register` | Permission: `directory.people.register` | Idempotent<br>Audited |
| Resource | `directory.people` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `fullName`, `email`, `employmentType`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/user-directory/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/user-directory/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["directory.people.register"]
  action --> validation["Schema + permission guard"]
  validation --> service["User Directory service layer"]
  service --> state["directory.people"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, registerPersonAction } from "@plugins/user-directory";

console.log("plugin", manifest.id);
console.log("action", registerPersonAction.id);
```
